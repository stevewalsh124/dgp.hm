...
for(run in 1:32){
  ...
  for (i in 0:15) {
    if(model > 10) folder = "pow.ic"
    if(model > 19) folder = "pow_2.ic"
    if(model > 29) folder = "pow_3.ic"
    if(model > 39) folder = "pow_4.ic"
    mod <- read.table(paste0("../CosmicEmu_etc/32 Models Mira Titan/",folder,"/M0", 
...
plot(log10(k_sub), log10(var_pkms),
     main = paste("log10(k) vs log10(Var(P(k))), model", model))
