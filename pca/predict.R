...
###############################################################################
# This script obtains batch predictions and 
# main effects estimates, and ANOVA decomposition of the variance
#
...
mtext(paste0('all (',ncol(bases),') bases'),side=3,lin=.2)
# use n_pc bases
matplot(kvals,bases[,1:n_pc]%*%t(coef[,1:n_pc])+mean0mat,type='l',ylab='P(k)')
mtext(paste0(n_pc,' bases'),side=3,lin=.2)
