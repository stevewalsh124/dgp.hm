...
matrices $\Sigma_A$ and $\Sigma_B$ have a Mat\'ern covariance function with a nugget of 
$10^{-8}$ and smoothness of 2.5. We obtain function realizations at $x=\{0, 0.1, \dots, 
3.9, 4.0\}$, so that each function draw contains $n=41$ observations. We draw $r$-many 
realizations from a GP with some mean ($f_1$ or $f_2$) and some spatially varying covariance
...
where $k(\cdot,\cdot)$ is defined in Section \ref{sec:mod_spec}.  Within $\Sigma_B$, $s$ introduces
nonstationarity into the function realizations, more accurately matching the behavior of the 
CAMB and Mira-Titan motivating datasets.
...
\begin{figure}
    ...
    \caption{Illustration of simulation output, each with $r=5$ function realizations. The true function
             ...
             Left panel: function $f_1$, variance setting $A$. 
             Right panel: function $f_2$, variance setting $B$.}   
    \label{fig:run_sim_f1A_f2B}
\end{figure}

For each simulated function and variance specification, we consider 20 Monte Carlo repetitions
with re-randomized $m_1, m_2, u_1, u_2$.  We consider $r = 5$ and $r = 15$. Examples of such
simulation output are shown in Figure \ref{fig:run_sim_f1A_f2B}.
...
Emu \citep{moran2023mira}. These competing methods view the inputs as noisy observations
(i.e., the points in Figure \ref{fig:run_sim_f1A_f2B}) with no spatial dependence. In
contrast, our method uses the $r$ functional realizations to estimate parameters
within $\Sigma_\epsilon$ that account for the spatial dependence.

The metric we highlight to evaluate performance of each method is the
log score, a proper scoring rule which takes both prediction and intervals into 
consideration \citep{gneiting2007strictly}. A lower value indicates 
...
Mean squared error (MSE) results, which do not address UQ, were comparable across 
competing methods; these results can be found in Figure \ref{fig:sims_MSE}.
...
CAMB dataset contains data for sixty-four cosmologies, with each containing multiple 
(fifteen) low-resolution spectra, and one high-resolution spectrum. 
Unlike the Mira-Titan suite, this dataset additionally contains an infinite-resolution 
...
of credible intervals. An illustration of the first cosmology from CAMB is shown
in Figure \ref{fig:fit_camb}, where the infinite-resolution spectrum is generally
much smoother than the low- and high-resolution counterparts.
...
Our goal here is to train the DGP.FCO model and accurately estimate the true spectrum for each
cosmology. To imitate the role of the perturbation theory output of Mira-Titan, we anchor 
estimates of the CAMB power spectrum at the infinite-resolution for $\log_{10}(k) < -1.7$, and 
then rely strictly on the low- and high-resolution runs for $k$ above this (note the transition
area in Figure \ref{fig:fit_camb} around the transformed $k$ value of 0.2). We first use a log-log 
regression model to obtain precision estimates $\Lambda_\ell$ and $\Lambda_h$
for the low- and high-resolution runs respectively, as outlined in Section \ref{sec:data}. 
Similarly, we set $\Lambda_c^{(ii)}=10^8$ when $\log_{10}(k_i) < -1.7$ ($0$ otherwise) for $i = 1,\dots,n$.
\textcolor{red}{(why switch from $10^5$ to $10^8$?)} 
to illustrate high confidence in the infinite-resolution run for low $k$ values.
We then take a weighted average of the CAMB runs in a similar fashion
to Mira-Titan data as described in Section \ref{sec:data}: $\bar y = \Lambda^{-1}(\Lambda_c y_c + 
\Lambda_{\ell} \bar{y}_\ell + \Lambda_h y_h)$, where $\Lambda = \Lambda_p + \Lambda_\ell + \Lambda_h$ 
and $\bar{y}_\ell = \frac{1}{15}\sum_{r=1}^{15} y_{\ell_r}$.

With weighted average and precision information, we considered a Mat\'ern kernel for 
$\Sigma_\varepsilon$. 
\textcolor{red}{(Yes, using maximum likelihood estimation with smoothness set to $5/2$, 
and we use $\tau^2$ and $\theta$ estimates here, in the sims, and for Mira-Titan; 
these estimates are coming out of the ``opt\_matern" function we created.)}
For the CAMB data, spatial correlation decays very quickly
($\theta\approx 0$ for CAMB, compare the smoothly varying low-resolution runs of 
Figure \ref{fig:fit_camb} with the jagged low-resolution CAMB runs in Figure \ref{fig:plot_data}). 
We therefore assume no spatial dependence for CAMB and specify $\Sigma_\varepsilon = 
...
\textcolor{red}{As of now, we are not comparing to DPC/hetGP/deepgp here, so no
MSE or log scores. We do have MSEs of CAMB predictions when training on these posterior means
vs training on the infinite-resolution runs; this comes up in Figures \ref{fig:pca_preds_v_camb} 
and \ref{fig:mse_camb} later.}
...
matrix $\Sigma_\varepsilon$ (accounting for domain knowledge and spatial dependence) 
may outperform a diagonal covariance $\Lambda_\ell^{-1}$ for $\bar Y$. From Section 
\ref{sec:data}, we can incorporate domain knowledge from $\Lambda_p, \Lambda_\ell,$ 
and $\Lambda_h$ within our model, and calculate the weighted average for each cosmology: 
$\bar y = \Lambda^{-1}(\Lambda_p y_p + \Lambda_{\ell} \bar{y}_\ell + \Lambda_h y_h)$, 
where $\Lambda = \Lambda_p + \Lambda_\ell + \Lambda_h$. 
In previous work, we have modeled multiple candidates for 
...
The UQ from the DGP.FCO model accounts for varying precisions across differing $k$ 
values, as well as the spatial correlation within the functional realizations.
Figure \ref{fig:plot_warp} provides elliptical slice samples of $W$ along with the 
average warping. From these samples, we see some departure from stationarity 
estimated in the higher $X$ (i.e., scaled $k$) values.
