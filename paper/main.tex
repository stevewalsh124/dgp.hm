...
is valid (center panel), we see both low-resolution and high-resolution runs oscillate around 
...
random variable within the statistical model.  Together, $X$ and $\bar{Y}$ comprise the 
training data for a particular cosmology. $\bar{Y}$ is typically pre-processed
based on domain expertise, such as that defined in Section \ref{sec:data}.
We will reserve details of domain-specific pre-processing for the CAMB and Mira-Titan suites
...
this, we specify a dense covariance matrix $\Sigma_\varepsilon$ that simulataneously
allows us to model the correlated errors, as well as build domain-matter expertise
...
nonstationarity by warping $X$ via $W \sim \mathrm{GP}\left(\boldsymbol{\mu}_W, \Sigma_W(X)\right)$.
...
We use the Mat\`ern kernel \textcolor{red}{@Annie, might need to add/remove $\tau^2$ 
in one or both of these kernels},
\[
\begin{aligned}
...
\end{aligned}
\]
In the $S$ layer, hyperparameters $\tau^2$ and $\theta_S$ govern the correlation 
scale and lengthscale respectively.  In the $W$ layer, $\Sigma_W(X)^{i,j}$ is defined 
analogously for squared distances $d=||x_i-x_j||^2$ and we allow for a unique 
lengthscale $\theta_W$ but enforce unit scale following \citet{sauer2023active}.
...
W &\sim \mathrm{GP}\left(\mathbf{0}, \Sigma_W(X)\right)
\end{align}
Details are provided in \ref{sec:apdx_int_lik}. The incorporation of $\Sigma_\epsilon$ in the
...
We embrace fully-Bayesian inference of latent $W$ and kernel hyperparameters $\{\tau^2, \theta_S, \theta_W\}$.
...
Under a reference prior $\pi(\tau^2)\propto\frac{1}{\tau^2}$,
we integrate $\tau^2$ from the likelihood of Eq.~(\ref{eq:likelihood}).  The resulting marginal 
posterior is equivalent to that with the plug-in estimator 
$\hat{\tau}^2 = \dots$ \citep[][Chapter 5]{gramacy2020surrogates}.
\textcolor{red}{(With $\Sigma_\epsilon$ on there, we can't actually do this.. unless we divide
$\Sigma_\epsilon$ by $\tau^2$...)}.
