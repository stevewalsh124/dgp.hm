...
We consider $\Sigma_\varepsilon$ as known, given domain-matter expertise.
\textcolor{red}{(Here, we are fixing $\Sigma_\varepsilon$ upon estimating the Matern 
parameters and utilizing the precision estimates (domain-matter expertise from 
log-log regression model)?  The estimate of $\Sigma_\varepsilon$ is tailored for
each cosmology, as the Matern estimates are computed uniquely for each.)}
...
in \ref{sec:apdx_SgivenY}. With all posterior draws $S^{(t)}$, we compute
