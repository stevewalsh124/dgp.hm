...
a convoluted patchwork of potential spectra, which vary in fidelity and are trusted
...
\begin{figure}[ht]
    ...
    {\it Right:} Same as left, but with estimated posterior mean subtracted.
    \textcolor{red}{Rerun on Windows for $\mathcal{P}$.  Make axis/labels and legend 
    a bit larger so they are easier to read.  Am I tripping or are the horizontal lines
    in the left panel not lined up correctly?}}
    \label{fig:plot_data}
\end{figure}
...
Figure \ref{fig:plot_data} \textcolor{red}{(This plot goes back to the original range,
would it be prettier and easier for the reader to follow if we kept it to the 
range of the middle panel here?)}

Notice, in the center panel of Figure \ref{fig:plot_data}, the precision of each
spectra increases as $k$ increases.  The curves get progressively tighter from left
to right.  We would like to conveniently wrap this domain-specific knowledge inside
our Bayesian framework later.  To this end, \cite{moran2023mira} 
used the multiple low- and high-resolution spectra across all cosmologies to
obtained precision estimates of these spectra across the $n$ 
...
To streamline notation, we denote three unique diagonal matrices:
\[
\Lambda_p = 
   \begin{tikzpicture}[decoration={brace,amplitude=5pt},baseline=(current bounding box.west)]
     ...
     \draw[decorate] (magic-1-1.north) -- (magic-3-3.north east) node[above=5pt,midway,sloped] {$k < 0.04$};
   \end{tikzpicture}
\quad
\Lambda_l = 
   \begin{tikzpicture}[decoration={brace,amplitude=5pt},baseline=(current bounding box.west)]
     ...
     \draw[decorate] (magic-4-4.north) -- (magic-6-6.north east) node[above=5pt,midway,sloped] {$0.04\leq k < 0.25$};
   \end{tikzpicture}
\]
\[
\Lambda_h = 
   \begin{tikzpicture}[decoration={brace,amplitude=5pt},baseline=(current bounding box.west)]
     ...
     \draw[decorate] (magic-4-4.north) -- (magic-6-6.north east) node[above=5pt,midway,sloped] {$0.25\leq k \leq 5$};
   \end{tikzpicture}
\]
\textcolor{red}{(@Steve - I am still quite confused here.  Are the above matrices helpful?
I'm confused by $p_i$.  Isn't there some overlap
between the low and high res runs?  The piecewise form you had doesn't have any overlap.  You
had the large $\Lambda$ as the concatenation of the matrices, but then you also had
it as a sum.  Perhaps we should talk this out sometime.  There has to be an easier 
way to convey how we calculate the weighted average.  Perhaps we could represent 
this better with indicator functions?  
Or as a function of $k_i$?  The equations below are just some ideas.)}
\[
\Lambda_p(k) = 10^5 \mathbbm{I}_{\{k < 0.04\}}
\]
where $\mathbbm{I}$ is the indicator function.
\[
\Lambda(k_i) = \begin{cases}
    10^5 \text{ for } k_i < 0.04 \text{  (perturbation theory)}\\
    16p_i \text{ for } 0.04 \leq k_i < 0.25 \text{  (low resolution)}\\
    cp_i \text{ for } 0.25 \leq k_i \leq 5 \text{  (high resolution)}
    \end{cases},
\]
where we use the sufficiently high precision of $10^5$ to impose the deterministic
nature of the perturbation theory output.  \textcolor{red}{(Then comment on how 
we can put this all into one big diagonal matrix that we multiply to our y values?)}



% $\Lambda_p = \text{diag}\left((10^5)_{k < 0.04}\right)$, 
% $\Lambda_{\ell} = \text{diag}\left(16\times(p_1,\dots,p_n)_{0.04 \leq k < 0.25}\right)$, 
% $\Lambda_h = \text{diag}\left(c\times(p_1,\dots,p_n)_{0.04 \leq k \leq 5}\right)$. 
...
%=
%\begin{bmatrix}
%\operatorname{diag}(10^5, \ldots, 10^5) &  &  \\
% & \operatorname{diag}(16\times(p_1, \ldots, p_n)) &  \\
% &  & \operatorname{diag}(c\times(p_1, \ldots, p_n))
%\end{bmatrix}
%\]

\textcolor{red}{(This part will need some revising, it is still unclear to me
how the weighted average is calculated.  Although I know in reality it is super simple.}
Above, each matrix element is either the corresponding precision value or zero, 
depending on whether the $k$ value is approximately unbiased for that data type. 
With this, we calculate a weighted average for each cosmology: 
...
the weighted average for a particular cosmology. On its own, the weighted average
has too much variation baked in from the limited number of model runs. There is also
no spatial dependence accounting for the correlated errors seen in Figure \ref{fig:plot_data}.
To account for these modeling challenges, we need a more complex model.
...
independent of the others.  Let $D_n = \{X_n, \bar{Y}, \Lambda\}$ represent the aggregated 
data for a particular cosmology.  Let $S$ represent the true (but unknown) underlying
power matter spectra from which $D_n$ was generated.
\textcolor{red}{(What is X?  Why is $\bar{Y}$ capital here, but it wasn't earlier?
Since $\Lambda$ is fixed and doesn't change, I do not think we need to consider it part
of the data.)}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{CAMB_fit_model1.jpg}
