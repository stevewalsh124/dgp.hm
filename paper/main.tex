...
matter spectrum, Figure \ref{fig:plot_warp} shows the elliptical slice samples of $W$.
These warpings depart from the identiy mapping around $x=0.4$ \textcolor{red}{(@Steve - 
please change the x-axis to match the actual x-axis values, then adjust this number)}, 
...
\begin{figure}[ht]
    ...
    \textcolor{red}{(@Steve - combine this plot with Figure 6.  The two panels of Figure 6
    are redundant and unecessary (yet oddly, they don't match?).  Have this Mira-Titan 
    plot take up 2/3 of the space, then have just the right panel from Figure 6 take 
    up the remaining 1/3.  Update the figure caption and referencing accordingly.)}}
    \label{fig:plot_fit}
\end{figure}
...
\begin{figure}[ht]
   ...
   \caption{Left: MCMC samples of $W$ for model M001 after burnin and thinning. 
            ...
            \textcolor{red}{(remove this figure, combine right panel with previous)}}
   \label{fig:plot_warp}
\end{figure}
