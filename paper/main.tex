...
components (PCs; \textcolor{red}{@Steve - can you add a textbook reference?}).  
We first center all $\hat{S}_j$ by subtracting $\bar{S} = \frac{1}{111}\sum_{j=1}^{111} \hat{S}_j$, then
we combine them into an $n\times m$ matrix $\boldsymbol\eta$.  We use principal components to decompose 
$\boldsymbol\eta = BW^\top$ where $B$ is the $n\times p_\eta$ matrix of basis functions and $W$
is the $m\times p_\eta$ matrix of basis weights.  Details on this decomposition are 
reserved for \ref{sec:apdx_basis}.  For the CAMB dataset, $p_\eta=6$; for Mira-Titan, $p_\eta=8$.
As a visual, the left panel of Figure \ref{fig:mean_PCs_oneW} shows $\bar{S}$ for the Mira-Titan 
training cosmologies.  The center panel shows the $p_\eta=8$ basis functions.
...
\begin{figure}
    ...
    illustration of how the weights for the first PC will change as the first 
    cosmological parameter varies \textcolor{red}{This last plot is wonky because 
    it was made with only the $N=6$ test cosmologies, not a large uniform or FF design, 
    Steve can update this}.
    \textcolor{red}{This figure also needs work.  Axes need to be fixed.  Caption needs re-write.
    Right panel needs to be redone.  It should look smooth like a GP.  Or else just remove it.}}
    \label{fig:mean_PCs_oneW}
\end{figure}

 
Let $w_i^{(j)}$ denote the $ij^\textrm{th}$ element of $W$, representing the basis weight for the 
$j^\textrm{th}$ cosmology and the $i^\textrm{th}$ PC.
Conditional on $B$, our training data now consists of scalars $w_i^{(j)}$ as a function of $\psi_j$
for $j \in \{1,\ldots,m\}$ and $i \in \{1,\ldots,p_\eta\}$.  If we can effectively predict these basis weights
as a function of $\psi$ (i.e., $\hat{w}_i(\psi)$ for $i \in \{1,\ldots,p_\eta\}$), then we may obtain predictions 
of the power spectrum as
\[
\hat{S}(\psi) = \sum_{i=1}^{p_\eta} \hat{w}_i(\psi) B_i,
\]
...
SEPIA \citep{gattiker2020sepia}. That is, each $w_i(\psi)$ is modeled as a 
...
\textcolor{red}{(@Steve - there is a lot missing here.  The inputs to the covariance kernel
...
Using these trained GPs from the PCs, we predict the spectra for each of the $N$ held-out 
cosmologies for both the CAMB and Mira-Titan datasets. \textcolor{red}{(@Steve - this needs work too.
Please include the formula for the GP posterior mean, which is how we obtain $\hat{w}_i$.  We never
had use to include it earlier, but we should use it here.  If you need, you can introduce new matrix/vector
notation to represent the training data in this scenario.  At this point, you should refer back to the right
panel of Figure \ref{fig:mean_PCs_oneW} and narrate how this shows the predicted GP mean for $w_1$ (for Mira-Titan)
as a function of the first cosmological parameter $\psi_1$.)}

\subsection{Predicting Spectra for CAMB}
\label{subsec:camb_pred}

\textcolor{red}{(@Steve - we removed the text earlier that talked about the training/testing split in the
wrong section.  You need to introduce it here now.)} From our $m=32$ posterior means using DGP.FCO, 
these are input into SEPIA \textcolor{red}{(What does this mean?  Just refer to the equations from the previous
section for how we obtain predictions.)} in order to predict power spectra
for the $N=32$ test cosmologies.
...
from the PC decomposition.  Figure \ref{fig:pca_preds_v_camb} compares our predictions (DGP.FCO + PC, red)
to this ``best-case'' prediction (true + PC, blue) and the actual true spectrum for \textcolor{red}{10?} 
of the held-out test cosmologies.  There is a high level of agreement between the pairs of predictions 
across cosmologies.  We then compare the mean squared errors of our predictions and the ``best-case'' 
...
\begin{figure}
    ...
    \caption{Predictions of power matter spectra for 16 of the CAMB test cosmologies,
             ...
             \textcolor{red}{(This figure does not look polished.  Make it wider.  I recommend
             showing 2 rows of 5 plots.  Also, change line types so we are less reliant on color.
             Missing x-axis label.  No need to number the panels.)}}   
...
\begin{figure}
    ...
    \caption{Comparison of MSE for CAMB predictions, when training with DGP.FCO posterior 
             ...
             \textcolor{red}{(This figure also needs refinement.)}}   
    \label{fig:mse_camb}
\end{figure}
...
\textcolor{red}{(This sentence is misleading.... we only offer one figure that shows RMSE.)}
...
\citep{moran2024dpc}.  \textcolor{red}{(Add sentence about hwo Cosmic Emu also uses a PC
approach to predict power matter spectra for held-out cosmologies as a function of $\psi$.)}
...
%performance between Cosmic Emu and DGP.FCO for the $N=6$ held-out cosmologies. 
...
Using our proposed approach and Cosmic Emu, we obtain two sets of predictions for $\hat{S}_t(\psi_t)$ 
with $t\in\{1,\ldots,6\}$ indexing the six held-out cosmologies.  To summarize and compare
performance, we evaluate the root mean squared error across $t$ for each wavenumber $x_i = \log_{10}(k_i)$.
\textcolor{red}{(This is again missing key details.  What is the truth you are using to calculate
RMSE???)}  The left panel of Figure \ref{fig:plot_rmse_k} shows these RMSEs for the held-out cosmologies.
Comparisons of the spectrum estimation for training cosmologies are shown as well (using RMSE against 
the weighted average of observations). \textcolor{red}{(What does this previous sentence mean??? We
have discussed that the weighted average is not a good estimate of the true spectrum.  So we should not
be comparing our estimated means to the weighted averages.  I am so confused....)}

For predictions, DGP.FCO achieves the greatest 
...
cosmology are shown in Figure \ref{fig:plot_pred_1to6}.  \textcolor{red}{(Why is this figure in the appendix?
But even more concerning, why are we subtracting the weighted average?  In every other plot we subtract our
estimated mean.  This switch to using the weighted average in the final subsection is very concerning
to me.  Also - what is the reader supposed to be gleaning from each of these plots?  Any time you
have a figure, you need to include narration in the text that explains what the takeaways are.)}

\begin{figure}[ht]
    \centering
    \includegraphics[width=6in]{rmse_by_k.png}
    \caption{{\it Left:} Average RMSE of each prediction method across all $k$ values. 
             {\it Right:} Average RMSE for each method when using the held-out computer 
             model runs to train the model. In each case, the truth is assumed to 
             be the weighted average of the computer model runs.
             \textcolor{red}{(Shouldn't the average be across the 6 cosmologies?  Saying it 
             is across the $k$ values is misleading.  Again, I am concerned for us to use
             the weighted average as the truth.  This seems to go starkly against things
             that we have said earlier in the paper.)}}
...
\textcolor{red}{(@Steve - I think you should revisit this section once the rest of the
revisions are done.  Make sure it aligns with what the paper ends up being.  I am not going
through this sectio myself at this time.)}
...
% \textcolor{magenta}{Steve The different appendices from dissertation as necessary. 
...
\textcolor{red}{(@Steve - revise the text so it works for any $m$, not just 111.  
Also, replace $\Gamma$ with $W$ so it matches the text I revised earlier.)}
...
For this application, $\boldsymbol\eta$ has rank $r=\text{min}\{m,n\}$ (111 for Mira-Titan, 
32 for CAMB), which determines the number of non-zero elements in $D$. We find an alternative 
