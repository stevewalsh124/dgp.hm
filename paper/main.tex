...
W &\sim \mathrm{GP}\left(\mathbf{0}, \Sigma_W(X)\right)
\end{align}
Details are provided in Appendix \textcolor{red}{???}. We then use elliptical slice
...
\textcolor{red}{(@Steve - check the version number of the package in this reference.  Also,
I'm not sure what we really do with $\tau^2$.  In my package, we assign it a reference prior and
integrate it out.  I don't think we can do that here since we have the $\Sigma_\varepsilon$.)}
%\textcolor{red}{Annie, please ensure the most up-to-date 
...
\end{array}
\] 
\textcolor{red}{(@Steve - provide details of this derivation.  Also what is $\mu$?  Are we 
going with zero here?  Add details of how we combine these things across all the $t$ samples.)}
...
\textcolor{red}{(Is it too late to change this?  Maybe DGP-FCO for functional correlated output?)}
...
$$
\begin{aligned}
Y_i|S,W &\sim N\big(S, \Sigma_\varepsilon(w,w')\big), \quad i \in \{1,\dots, r\}\\
S|W &\sim N\big(m, \Sigma_S(w,w')\big)\\
W &\sim N\big(0, \Sigma_W(x,x')\big) \\
\end{aligned}
$$

We can obtain the integrated likelihood $p(Y_i|W)=\int p(Y_i|S,W)p(S|W) dS$, with the following calculations:
$$
\begin{aligned}
p(Y_i|W) &= \int p(Y_i|S,W)p(S|W)dS \\
&\propto \int\exp\left(-\frac{1}{2} \left[Y_i^T\Sigma_\varepsilon^{-1} Y_i - 2Y_i^T\Sigma_\varepsilon^{-1} S + S^T \Sigma_\varepsilon^{-1} S + S^T \Sigma_S^{-1} S -2S^T \Sigma_S^{-1} m + m^T \Sigma_S^{-1} m \right]\right)dS \\
\end{aligned}
$$
...
$$
\begin{aligned}
    \exp\Big(-\frac{1}{2} \big[S^T &(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1}) S -2S^T (\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i) \big]\Big) \\
&= \exp\left(-\frac{1}{2} \left[S^T (\Sigma_\varepsilon^{-1}+\Sigma_S^{-1}) S -2S^T (\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}(\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i) \right]\right)
\end{aligned}
$$

To complete the square, we add and subtract $(\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i)^T(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}(\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i)$, which then gives the kernel for $S \sim N\left((\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}(\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i), (\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1} \right)$. This removes the integral and leaves us with the following terms for $Y_i|W$:
$$
\begin{aligned}
&\exp\Big(-\frac{1}{2}\big[Y_i^T\Sigma_\varepsilon^{-1} Y + m^T\Sigma_S^{-1} m - (\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i)^T(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1} (\Sigma_S^{-1} m + \Sigma_\varepsilon^{-1} Y_i)\big]\Big) \\
&= \exp\Big(-\frac{1}{2}\big[Y_i^T\big(\Sigma_\varepsilon^{-1} - \Sigma_\varepsilon^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_\varepsilon^{-1}\big)Y_i -2Y_i^T \Sigma_\varepsilon^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_S^{-1} m \\
&\qquad \qquad \qquad + m^T (\Sigma_S^{-1} - \Sigma_S^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_S^{-1}) m\big]\Big) \\
&= \exp\Big(-\frac{1}{2}\big((Y_i-m)^T(\Sigma_\varepsilon+\Sigma_S)^{-1}(Y_i-m) \big)\Big)
\end{aligned}
$$

With the last equality resulting from the use of the Sherman–Morrison–Woodbury formula, yielding $\big(\Sigma_\varepsilon^{-1} - \Sigma_\varepsilon^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_\varepsilon^{-1} = (\Sigma_S^{-1} - \Sigma_S^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_S^{-1}) = (\Sigma_\varepsilon+\Sigma_S)^{-1}$. We also use the fact that $\Sigma_\varepsilon^{-1}(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})^{-1}\Sigma_S^{-1} = (\Sigma_\varepsilon + \Sigma_S)^{-1}$, which holds since it can be shown that $[\Sigma_S(\Sigma_\varepsilon^{-1}+\Sigma_S^{-1})\Sigma_\varepsilon](\Sigma_\varepsilon+\Sigma_S)=(\Sigma_\varepsilon+\Sigma_S)(\Sigma_\varepsilon+\Sigma_S)$.

Therefore, we have that $Y_i|W \sim N\big(m, \Sigma_S(w,w')+\Sigma_\varepsilon(w,w')\big)$.
