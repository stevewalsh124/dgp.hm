...
$\Lambda_\ell$, and $\Lambda_h$ as the diagonal matrices with elements
\[
...
independent of the others. In the previous seciton, we used lowercase $\bar{y}$ to denote 
the observed weighted average; here, we use $\bar{Y}$ to indicate the corresponding 
random variable within the statistical model. Specifically, our input $X$ is the 
$n$ values of $\log_{10}(k)$ and the output is the weighted average $\bar{Y}$. 
Let $D = \{X, \bar{Y}, \Lambda\}$ represent the aggregated 
...
\textcolor{red}{(@Annie You suggested removing $\Lambda$ from $D$; is fixed and 
doesn't change here, but it would if $X$ or $Y$ changed. Also, we use $\Lambda$ to pre-scale the data)}
