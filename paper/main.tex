...
\begin{figure}[ht]
    ...
    of $W$, illustrating some nonstationary behavior.}
    \label{fig:plot_fit}
\end{figure}
...
Section \ref{sec:data}) as $\psi \in \mathbb{R}^{p_\psi}$. With our estimated posterior means
from DGP.FCO for each training cosmology, we now predict $S$ for unobserved cosmologies
as a function of $\psi$.

We have $m$-many training observations ($j = 1,\dots,m$) which each contain the $p_\psi$-dimensional
input $\psi_j$ and DGP.FCO estimates $\hat{S}_j$ for the $j$th cosmology. 
We combine all $\hat{S}_j$ estimates into a $n\times m$ matrix $\boldsymbol\eta$.
Due to the fact that each column of $\boldsymbol\eta$ represents functional output, 
we choose to reformulate the problem using principal components (PCs). Details of this can be found in 
\ref{sec:apdx_basis}. Here we will use the first $p_\eta$ PCs. 

Figure \ref{fig:mean_PCs_oneW} illustrates this decomposition with the estimated 
mean trend for the Mira-Titan dataset, the $p_\eta$ basis functions, and one 
example of how the weight for the first PC will vary dependent on the first of 
$p_\psi=8$ cosmological parameters contained in $\psi$. As a result, our goal becomes 
estimating the weights $\Gamma$ corresponding to each of the $p_\eta$ PCs, 
as a function of $\psi$.
...
cosmologies for both the CAMB and Mira-Titan datasets.

\subsection{Predicting Spectra for CAMB Data}
\label{subsec:camb_pred}

From our $m=32$ posterior means using DGP.FCO, these are input into SEPIA in order to predict power spectra
for the $N=32$ test cosmologies. In the CAMB dataset, $p_\psi=6$. With the infinite-resolution spectra, we 
have a sense of the ``best-case" predictions we can have (i.e., if we trained with the 32 true 
infinite-resolution spectra, as opposed to our estimated posterior means). In Figure \ref{fig:pca_preds_v_camb}, 
we see comparisons of the heldout infinite resolution alongside two predictions: one 
based on DGP.FCO and the ``best-case" scenario using the infinite-resolution spectra of 
the training set. There is a high level of agreement between the pairs of predictions 
across cosmologies. Upon comparing MSEs for each of these prediction scenarios 
...
\begin{figure}
    ...
    \caption{Predictions for 16 of the $N=32$ test cosmologies after centering and scaling. 
...
We compare our method with Cosmic Emu \citep{moran2023mira}, the emulator constructed using the full suite 
of $m=111$ Mira-Titan model runs. Cosmic Emu uses a Bayesian approach where the smooth 
...
$N=6$ cosmologies and each $k$ value are shown in Figure \ref{fig:plot_rmse_k}. Comparisons 
