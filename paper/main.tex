...
obtained precision estimates, $p_1,\dots, p_n$, across the $n$ 
wavenumber values using a log-log regression model.
% and a multiplier $c$ for the increase in precision from the 
% low- to high- resolution output.
We use these estimates to define three diagonal precision matrices, 
one for each data type (perturbation theory, low resolution, and high resolution).
Specifically, denote $\Lambda_p$, $\Lambda_\ell$, and $\Lambda_h$ as the
...
\textcolor{red}{(@Steve - what is $c$
exactly?)}  We then calculate a weighted average for each cosmology: 
...
underlying spectra, the weighted average fluctuates too drastically due 
to the limited number of model runs.  It fails to account for the
