...
\begin{figure}
    ...
    \caption{Example of DGP.FCO model fit for the first batch of model runs from the CAMB data, 
             ...
             \textcolor{red}{(@Steve - can you leave the x-axis on the usual scale?
             So -1.7 is directly visible.)}}   
    \label{fig:fit_camb}
\end{figure}
...
$\log_{10}(k) < -1.7$, and then rely strictly on the low- and high-resolution 
runs for $k$ above this (note the transition
in Figure \ref{fig:fit_camb} at $\log_{10}(k) = -1.7$). To compute $\bar{y}$ and $\Sigma_\epsilon$,
...
$\Lambda_c^{(ii)}=10^8$ for $\log_{10}(k_i) < -1.7$ ($0$ otherwise).
...
($\log_{10}(k)\approx 0.4,0.7$ - \textcolor{red}{@Steve - please adjust these numbers after 
changing the x-axis of the figure}). Our model also provides effective UQ, appropriately shrinking
