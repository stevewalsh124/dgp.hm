...
\textcolor{cyan}{Annie, please include the most up-to-date prior specs here.}
...
To being, we initialize our hyperparameter estimates across all cosmologies by modeling the first (M001) with 50,000 MCMC runs, and use these as initial values for all cosmologies. Specifically, we save the last MCMC iteration of $W$, $\theta_W$ and $\theta_S$ as $W_0$, $\theta_W0$ and $\theta_S0$ respectively and use each as starting values for all 111 training cosmology fits, where these are now fit with 10,000 MCMC runs (5,000 removed as burn-in and every 5th sample is kept, resulting in 1,000 posterior samples).
